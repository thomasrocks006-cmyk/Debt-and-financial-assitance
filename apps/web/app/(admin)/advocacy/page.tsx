...
export default function AdvocacyPage() {
  ...
  async function handleGenerateLetter(templateId: string) {
    ...
    try {
      const res = await fetch("/api/advocacy/letter", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ templateId }),
      });
      const data = await res.json();
      setGeneratedLetter(data.letter ?? "Template not found.");
    } catch {
      setGeneratedLetter("Failed to generate letter. Please try again.");
    } finally {
      setGenerating(false);
    }
  }
